...
export class MigrationWorker {
   ...
   */
  async migrate(task: MigrationTask | ExtendedMigrationTask): Promise<MigrationResult> {
    const operations: MigrationOperation[] = [];
    const extTask = task as ExtendedMigrationTask;
    ...
    try {
      ...
      if (needsMove && this.pathMapping.size > 0) {
        const updateResult = this.referenceUpdater.updateLinks(
          migratedContent,
          task.sourcePath,
          this.pathMapping,
          task.targetPath
   ...
   */
  async migrateWithoutAI(task: MigrationTask | ExtendedMigrationTask): Promise<MigrationResult> {
    const operations: MigrationOperation[] = [];
    const extTask = task as ExtendedMigrationTask;
  ...
  private getSchemaForType(docType: string): object | null {
    ...
    if (!this.parsedSchemas) {
      try {
        this.parsedSchemas = safeLoad(this.schemasYaml) as Record<string, unknown>;
      } catch {
        return null;
      }
    }
    ...
    if (
      ...
      return this.parsedSchemas[schemaName] as object;
    }
    ...
    if (schemas && typeof schemas === 'object' && schemaName in schemas) {
      return schemas[schemaName] as object;
    }
...
Output ONLY the YAML frontmatter (including --- markers):`;
   ...
   * @returns MigrationResult with outcome
   */
  private async createNewFile(task: ExtendedMigrationTask): Promise<MigrationResult> {
    const operations: MigrationOperation[] = [];
   ...
   */
  private generateFrontmatter(docType: string, title: string): string {
