...
export class SchemaDesignStage {
  ...
  async execute(blueprint: string): Promise<SchemaDesignResult> {
    try {
      ...
      const iterations = (agentResult.metadata?.iterations as number) ?? 1;
        ...
        for (const [name, schema] of Object.entries(schemas)) {
          ...
          if (schemaObj._extends) {
            ...
            if (!schemaNames.includes(extendsName)) {
              errors.push(
                `Schema "${name}" extends unknown schema "${extendsName}"`
              );
            }
          }
          ...
          if (!schemaObj.properties) {
            ...
            for (const [propName, propDef] of Object.entries(properties)) {
              ...
              if (!/^[a-z][a-z0-9_-]*$/.test(propName)) {
                errors.push(
                  `Invalid property name "${propName}" in schema "${name}"`
                );
              }
              ...
              if (typeof propDef !== 'object' || propDef === null) {
                errors.push(
                  `Property "${propName}" in schema "${name}" must be an object`
                );
                continue;
              }
              ...
              if (!propObj.type) {
                ...
                if (!validTypes.includes(String(propObj.type))) {
                  errors.push(
                    `Invalid type "${propObj.type}" for property "${propName}" in schema "${name}"`
                  );
                }
              }
            }
