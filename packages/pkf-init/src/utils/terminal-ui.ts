...
export class TerminalUI {
  ...
  private totalStartTime: number = Date.now();
  private tokens: TokenUsage = { input: 0, output: 0, cacheCreation: 0, cacheRead: 0 };
  ...
  private drawHeader(): void {
    ...
    const topLine = COLORS.borderHighlight(
      BOX.DOUBLE_TOP_LEFT +
  ...
  addTokens(input: number, output: number, cacheCreation?: number, cacheRead?: number): void {
    ...
    if (cacheCreation) this.tokens.cacheCreation += cacheCreation;
    if (cacheRead) this.tokens.cacheRead += cacheRead;
    this.renderStatusBar();
  }
  ...
  setTokens(usage: Partial<TokenUsage>): void {
    if (usage.input !== undefined) this.tokens.input = usage.input;
    if (usage.output !== undefined) this.tokens.output = usage.output;
    if (usage.cacheCreation !== undefined) this.tokens.cacheCreation = usage.cacheCreation;
    if (usage.cacheRead !== undefined) this.tokens.cacheRead = usage.cacheRead;
    this.renderStatusBar();
  }
   ...
   */
  drawBox(title: string, content: string[], style: 'single' | 'double' = 'single'): void {
...
export function createTerminalUI(config: TerminalUIConfig): TerminalUI | SimpleProgress {
 ...
 */
export function createNoOpUI(): TerminalUI | SimpleProgress {
  return {
    start: () => {},
    stop: () => {},
    nextStep: () => {},
