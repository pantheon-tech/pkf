...
export class MigrationCleanup {
  ...
  async removeEmptyDirectories(
    ...
    const collectDirs = async (dirPath: string, depth: number = 0): Promise<void> => {
      try {
  ...
  async removeSourceFiles(
    ...
    for (const move of verifiedMoves) {
      ...
      if (move.operation.type !== 'copy') continue;
      if (!move.operation.targetPath) continue;
   ...
   */
  async findDuplicates(operations: MigrationOperation[]): Promise<string[]> {
    const duplicates: string[] = [];

    for (const op of operations) {
      if (op.type !== 'copy' && op.type !== 'move') continue;
  ...
  async fullCleanup(
    moves: MoveResult[],
    options: CleanupOptions & { cleanupBackups?: string } = {}
  ): Promise<CleanupResult> {
