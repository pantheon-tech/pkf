...
export class Interactive {
     ...
     */
    async approveBlueprint(blueprint) {
        if (!this.enabled) {
     ...
     */
    async approveStructure(structure) {
        if (!this.enabled) {
            return true;
        }
        console.log('');
    ...
    async approveMigration(tasks) {
        ...
        if (tasks.length > 0) {
            ...
            for (const task of displayTasks) {
                console.log(`  ${chalk.gray(task.sourcePath)} ${chalk.white('->')} ${chalk.green(task.targetPath)}`);
     ...
     */
    async showProgress(stage, message, progress) {
        if (!this.enabled) {
            return;
        }
     ...
     */
    formatYamlPreview(yaml, maxLines = 50) {
