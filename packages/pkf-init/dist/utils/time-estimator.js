...
const TOKEN_ESTIMATES = {
    analysis: { input: 2000, output: 4000, model: 'sonnet' },
    design: { input: 3000, output: 5000, iterations: 3, model: 'sonnet' },
    implementation: { input: 1000, output: 3000, model: 'haiku' },
    migration: { inputBase: 500, output: 1000, model: 'haiku' },
};
/**
...
export class TimeEstimator {
     ...
     */
    static estimateAnalysis(fileCount) {
     ...
     * @returns Estimated time in minutes
     */
    static estimateDesign(complexity) {
        const multipliers = {
            low: 0.7,
            medium: 1.0,
            high: 1.5,
     ...
     * @returns Estimated time in minutes
     */
    static estimateMigration(docCount, workers) {
...
export class DryRunReport {
     ...
     * @returns Token count
     */
    async countTokens(content, model = 'claude-sonnet-4-5-20250929') {
        if (!this.client) {
     ...
     */
    async calculateActualDocTokens(markdownFiles) {
     ...
     */
    determineComplexity(fileCount, docTypes) {
    ...
    async analyze(rootDir) {
        ...
        const analysisCost = this.calculateCost(TOKEN_ESTIMATES.analysis.model, TOKEN_ESTIMATES.analysis.input, TOKEN_ESTIMATES.analysis.output);
        const designCost = this.calculateCost(TOKEN_ESTIMATES.design.model, TOKEN_ESTIMATES.design.input * TOKEN_ESTIMATES.design.iterations, TOKEN_ESTIMATES.design.output * TOKEN_ESTIMATES.design.iterations);
        const implementationCost = this.calculateCost(TOKEN_ESTIMATES.implementation.model, TOKEN_ESTIMATES.implementation.input, TOKEN_ESTIMATES.implementation.output);
        const docSize = this.estimateDocSize();
        const migrationCost = docCount > 0
            ? this.calculateCost(TOKEN_ESTIMATES.migration.model, (TOKEN_ESTIMATES.migration.inputBase + docSize) * docCount, TOKEN_ESTIMATES.migration.output * docCount)
            : 0;
        // Calculate token estimates
        const analysisTokens = TOKEN_ESTIMATES.analysis.input + TOKEN_ESTIMATES.analysis.output;
        const designTokens = (TOKEN_ESTIMATES.design.input + TOKEN_ESTIMATES.design.output) *
            TOKEN_ESTIMATES.design.iterations;
        const implementationTokens = TOKEN_ESTIMATES.implementation.input + TOKEN_ESTIMATES.implementation.output;
        const migrationTokens = docCount > 0
            ? (TOKEN_ESTIMATES.migration.inputBase + docSize + TOKEN_ESTIMATES.migration.output) *
                docCount
            : 0;
        const estimatedTokens = analysisTokens + designTokens + implementationTokens + migrationTokens;
        ...
        const estimatedTimeMinutes = timeBreakdown.analysis +
    ...
    displayReport(estimate) {
        ...
        console.log(chalk.gray('─'.repeat(36)));
        console.log(`  Analysis:       ${chalk.green(this.formatCost(estimate.costBreakdown.analysis).padStart(10))}`);
        console.log(`  Design:         ${chalk.green(this.formatCost(estimate.costBreakdown.design).padStart(10))}`);
        console.log(`  Implementation: ${chalk.green(this.formatCost(estimate.costBreakdown.implementation).padStart(10))}`);
        console.log(`  Migration:      ${chalk.green(this.formatCost(estimate.costBreakdown.migration).padStart(10))}`);
        console.log(chalk.gray('─'.repeat(36)));
        console.log(`  ${chalk.bold('Total:')}          ${chalk.bold.green(this.formatCost(estimate.estimatedCost).padStart(10))}`);
        console.log();
        // Time breakdown section
        console.log(chalk.bold.white('Time Breakdown'));
        console.log(chalk.gray('─'.repeat(36)));
        console.log(`  Analysis:       ${chalk.blue(this.formatTime(estimate.timeBreakdown.analysis).padStart(10))}`);
        console.log(`  Design:         ${chalk.blue(this.formatTime(estimate.timeBreakdown.design).padStart(10))}`);
        console.log(`  Implementation: ${chalk.blue(this.formatTime(estimate.timeBreakdown.implementation).padStart(10))}`);
        console.log(`  Migration:      ${chalk.blue(this.formatTime(estimate.timeBreakdown.migration).padStart(10))}`);
        console.log(chalk.gray('─'.repeat(36)));
        console.log(`  ${chalk.bold('Total:')}          ${chalk.bold.blue(this.formatTime(estimate.estimatedTimeMinutes).padStart(10))}`);
        console.log();
