...
export class ReferenceUpdater {
     ...
     */
    calculateNewPath(linkFromFile, linkToFile, newLinkFromFile, newLinkToFile) {
        // Normalize paths
     ...
     */
    resolveLink(linkPath, fromFile, rootDir) {
     ...
     */
    updateLinks(content, fromFile, pathMapping, newFromFile) {
        ...
        for (const link of sortedLinks) {
            ...
            const newRelativePath = this.calculateNewPath(fromFile, resolvedTarget, effectiveFromFile, effectiveTargetPath);
     ...
     */
    buildCrossReferences(content, filePath, rootDir) {
        const links = this.findLinks(content);
        const refs = [];
     ...
     */
    validateLinks(content, filePath, allFiles, rootDir) {
        const links = this.findLinks(content);
        const broken = [];
        for (const link of links) {
            if (!this.isRelativeFilePath(link.path)) {
     ...
     */
    findSimilarFile(targetPath, allFiles) {
