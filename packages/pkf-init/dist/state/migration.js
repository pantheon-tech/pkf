 ...
 */
function parseMigrationKey(key) {
...
export function migrateState(state, targetVersion) {
    ...
    if (semver.gt(currentVersion, targetVersion)) {
        throw new Error(`Cannot downgrade state from ${currentVersion} to ${targetVersion}`);
    }
    ...
    const applicableMigrations = Object.keys(migrations)
        ...
        return (semver.lte(from, currentVersion) &&
...
export function validateState(state) {
    ...
    if (!Array.isArray(state.checkpoints))
        return false;
    if (typeof state.apiCallCount !== 'number')
        return false;
    if (typeof state.totalCost !== 'number')
        return false;
