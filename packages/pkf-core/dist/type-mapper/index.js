 ...
 * ```
 */
export const PKF_TYPE_TO_DIRECTORY = {
 ...
 * ```
 */
export function detectDocumentType(filePath, content) {
 ...
 * ```
 */
export function resolveTargetPath(sourcePath, docType, _rootDir, _docsDir = 'docs') {
 ...
 * ```
 */
export function getRequiredDirectories(docTypes) {
    const dirs = new Set();
 ...
 * ```
 */
export function shouldExcludeFromReorganization(filePath) {
    const normalizedPath = filePath.toLowerCase().replace(/\\/g, '/');
 ...
 * ```
 */
export const DOC_TYPE_TO_SCHEMA = {
