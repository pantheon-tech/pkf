 ...
 * ```
 */
export function validateSchemasYaml(schemas) {
    const errors = [];
    const warnings = [];
                ...
                for (const [propName, propDef] of Object.entries(properties)) {
                    ...
                    if (!propDef.type) {
                        ...
                        if (!validTypes.includes(String(propDef.type))) {
                            errors.push(`Invalid type "${propDef.type}" for property "${propName}" in schema "${name}"`);
                        }
                    }
                }
            }
    ...
    for (const [propName, propDef] of Object.entries(schema.properties)) {
        ...
        if (propName in data) {
            ...
            if (value !== null && value !== undefined) {
                ...
                if (!typeValid) {
                    errors.push(`Field "${propName}" has type ${actualType} but expected ${expectedType}`);
                }
                // Enum validation
                if (propDef.enum && propDef.enum.length > 0) {
                    if (!propDef.enum.includes(value)) {
                        errors.push(`Field "${propName}" value "${value}" is not in allowed values: ${propDef.enum.join(', ')}`);
                    }
                }
