...
export class SchemaLoader {
    ...
    extractSchemasYaml(response) {
        ...
        const indentedMatch = response.match(/(?:schemas\.yaml|Schema|schemas):\s*\n((?:[ ]{2,}|\t)[\s\S]*?)(?=\n[^\s]|\n\n[A-Z]|$)/i);
        if (indentedMatch && indentedMatch[1]) {
            // Remove common indentation
            const lines = indentedMatch[1].split('\n');
            const minIndent = Math.min(...lines.filter((l) => l.trim()).map((l) => {
